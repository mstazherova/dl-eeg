...
if __name__ == '__main__':
    ...
    model = ConvolutionalAutoEncoder.model() #load_model(args.model_file_name)
    ...
    for filter_index in filter_indexes:
        ...
        input_mel_data = np.random.random((1, input_width, input_height, 3))

        for i in range(20):
            loss_value, grads_value = iterate([input_mel_data])
            input_mel_data += grads_value

        out.append(input_mel_data.reshape(input_width, input_height, 3))

    np.save('introspection.npy', out)
